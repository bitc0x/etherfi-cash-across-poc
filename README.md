...
assets (USDY, sUSDe, sDAI, weETH, wstETH, USDS) — live on Ethereum. Across closes that gap: it
bridges USDC from Optimism to Ethereum and the requested asset is
delivered to the recipient (user wallet or Cash safe) — one signature on the atomic routes,
two on the async Fusion route. No ether.fi vault is deployed; the destination liquidity does
the swap.
